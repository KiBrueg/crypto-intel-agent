...
- Trader-style chart powered by TradingView Lightweight Charts when CDN is available; fallback schematic canvas if it is not. It includes horizontal price grid, right-side price scale, bottom time scale, OHLC header, current-price marker, volume bars, EMA9/EMA21/VWAP overlays, entry/stop/target lines, SMC/liquidity level lines and detected support/resistance/Fibonacci/pivot levels.
