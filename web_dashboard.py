...
<body><div class="wrap">
  ...
  <div class="grid">
    ...
    <div>
      ...
      <div class="card section" style="margin-top:16px"><h3>Learning Autopilot</h3><div id="autopilotstatus"></div><div id="forecasttimeline"></div><div id="learningautopilot"></div><button class="btn secondary" onclick="runLearningAutopilot()">Run learning cycle</button></div>
...
async function loadAutopilotStatus(){{try{{const [sr,tr]=await Promise.all([fetch('/api/autopilot/status'), fetch('/api/forecast/timeline?limit=8')]); const s=await sr.json(); const t=await tr.json(); renderAutopilotStatus(s); renderForecastTimeline(t);}}catch(e){{}}}}
