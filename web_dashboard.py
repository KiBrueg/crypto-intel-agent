...
function renderMiniMindCard(card){{const ai=card.ai_direction==='up'?'Growth':(card.ai_direction==='down'?'Fall':'Skip'); const user=card.user_direction?(card.user_direction==='up'?'Growth':(card.user_direction==='down'?'Fall':'Skip')):''; $('miniCardTitle').textContent=`${{card.symbol||'BTCUSDT'}} · ${{ai}} setup`; $('miniCardMeta').textContent=`${{card.exchange||'BINANCE'}} · ${{card.interval||'15m'}} · ${{card.mode||'mixed'}}`; $('miniCardChance').textContent=Math.round((card.ai_confidence||0)*100)+'%'; $('miniCardRisk').textContent='-'+fmt(card.risk_loss_pct)+'%'; $('miniCardRR').textContent=fmt(card.risk_reward_ratio); $('miniCardDesc').innerHTML=`${{user?('You: <b>'+user+'</b> · '+(card.agreement_with_ai?'AI view matches':'AI view differs')+'<br>'):''}}${{cardReasonsWithWorldNews(card).join(' ')}} <span class="mono">FOMO ${{Math.round((card.fomo_score||0)*100)}}%</span>${{user?mindResultComparison(card):''}}`; miniResultHint(card); drawMiniCardChart(card.chart_window||[]);}}
