...
function renderGlobalNewsImpact(g){{if(!g||g.error){{$('globalnewsimpact').innerHTML=`<p class="error">${{g&&g.error?g.error:'Global news scan failed'}}</p>`;return;}} const cls=g.crypto_bias==='bullish'?'good':(g.crypto_bias==='bearish'?'bad':'warn'); const stockCls=g.stocks_bias==='bullish'?'good':(g.stocks_bias==='bearish'?'bad':'warn'); const cards=(g.cards||[]).slice(0,8).map(c=>`<li><b>${{c.source||'News'}}</b>: ${{c.title}}<br><span class="small">Crypto: <b class="${{c.crypto_effect==='growth_pressure'?'good':(c.crypto_effect==='fall_pressure'?'bad':'warn')}}">${{c.crypto_effect}}</b> · Stocks: <b>${{c.stocks_effect}}</b> · ${{(c.categories||[]).join(', ')}} · ${{c.why}}</span></li>`).join(''); const watch=(g.what_to_watch||[]).map(x=>`<li>${{x}}</li>`).join(''); $('globalnewsimpact').innerHTML=`<p><b>${{g.headline}}</b></p><div class="metrics"><div class="metric"><div class="l">Crypto pressure</div><div class="v ${{cls}}">${{g.crypto_bias}}</div></div><div class="metric"><div class="l">Stocks pressure</div><div class="v ${{stockCls}}">${{g.stocks_bias}}</div></div><div class="metric"><div class="l">Categories</div><div class="v">${{Object.keys(g.category_counts||{{}}).length}}</div></div></div><h4>World headlines → market pressure</h4><ul class="list">${{cards||'<li>No high-impact world-news headline detected in scanned feeds.</li>'}}</ul><h4>Что смотреть на графике</h4><ul class="list">${{watch}}</ul>`;}}
