...
<head>
  ...
  <style>
    ...
    *{{box-sizing:border-box}} html{{background:var(--bg)}} body{{margin:0;min-height:100vh;background:radial-gradient(circle at 10% -12%,rgba(230,177,126,.36),transparent 34%),radial-gradient(circle at 88% 0%,rgba(168,95,61,.16),transparent 28%),linear-gradient(135deg,#fffdf8 0%,#fbf7ef 46%,#f2e6d6 100%);font-family:'Source Sans 3',system-ui,-apple-system,'Segoe UI',Roboto,sans-serif;color:var(--text);font-feature-settings:'ss01';}}
    ...
    @media(max-width:760px){{.mini-stage{{min-height:520px}}.mini-stage:before,.mini-stage:after{{display:none}}.mini-card{{width:100%}}.mini-chart{{height:220px}}}}
  </style>
</head>
<body><div class="wrap">
...
<script>
const $=id=>document.getElementById(id); let lastSnapshot=null, autoTimer=null, lastSetupId=null, currentMindCard=null, miniMindCard=null, miniHistory=[];
...
async function loadMiniMindCard(){{try{{const mode=$('mindcardmode').value||'mixed'; const r=await fetch('/api/mind-card/next?historical=1&mode='+encodeURIComponent(mode)+'&symbol='+encodeURIComponent($('symbol').value)+'&interval='+encodeURIComponent($('interval').value)); const card=await r.json(); if(card.error) throw new Error(card.error); miniMindCard=card; miniHistory.push(card); renderMiniMindCard(card);}}catch(e){{$('miniCardDesc').innerHTML='<span class="error">'+(e.stack||e)+'</span>';}}}}
async function miniChooseMindCard(direction){{try{{if(!miniMindCard){{await loadMiniMindCard(); if(direction==='skip') return;}} if(!miniMindCard||!miniMindCard.id)return; const size=direction==='skip'?'none':$('mindcardsize').value; const r=await fetch('/api/mind-card/choice',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{card_id:miniMindCard.id,direction,size}})}}); const answered=await r.json(); if(answered.error) throw new Error(answered.error); renderMiniMindCard(answered); if(direction==='skip'){{await loadMiniMindCard();}} else {{setTimeout(loadMiniMindCard,520);}}}}catch(e){{$('miniCardDesc').innerHTML='<span class="error">'+(e.stack||e)+'</span>';}}}}
...
function renderMiniMindCard(card){{const ai=card.ai_direction==='up'?'Growth':(card.ai_direction==='down'?'Fall':'Skip'); const user=card.user_direction?(card.user_direction==='up'?'Growth':(card.user_direction==='down'?'Fall':'Skip')):''; $('miniCardTitle').textContent=`${{card.symbol||'BTCUSDT'}} · ${{ai}} setup`; $('miniCardMeta').textContent=`${{card.exchange||'BINANCE'}} · ${{card.interval||'15m'}} · ${{card.mode||'mixed'}}`; $('miniCardChance').textContent=Math.round((card.ai_confidence||0)*100)+'%'; $('miniCardRisk').textContent='-'+fmt(card.risk_loss_pct)+'%'; $('miniCardRR').textContent=fmt(card.risk_reward_ratio); $('miniCardDesc').innerHTML=`${{user?('You: <b>'+user+'</b> · '+(card.agreement_with_ai?'AI view matches':'AI view differs')+'<br>'):''}}${{(card.ai_reason||[]).join(' ')}} <span class="mono">FOMO ${{Math.round((card.fomo_score||0)*100)}}%</span>`; drawMiniCardChart(card.chart_window||[]);}}
