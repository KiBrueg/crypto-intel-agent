...
function renderForecastTimeline(t){{if(!t||t.error)return; const last=t.last_forecast; const events=(t.events||[]).slice(0,6).map(e=>`<li><button class="chip" onclick="loadForecastDetail(${{e.id}})">#${{e.id}}</button> <b>${{e.symbol}}</b> ${{e.direction}}/${{e.status}} → <span class="${{e.display_status==='success'?'good':(e.display_status==='failed'?'bad':'warn')}}">${{e.outcome}}</span> <span class="small">entry ${{fmt(e.entry)}} · R ${{e.readiness_score||'n/a'}}</span></li>`).join(''); $('forecasttimeline').innerHTML=`<h4>Last AI Forecast</h4><p>${{last?('<b>#'+last.id+' '+last.symbol+'</b> '+last.direction+'/'+last.status+' → '+last.outcome):'No forecasts yet'}}</p><p class="small">Timeline: ${{(t.summary||{{}}).pending||0}} pending · ${{(t.summary||{{}}).success||0}} success · ${{(t.summary||{{}}).failed||0}} failed</p><ul class="list">${{events||'<li>No forecast timeline yet.</li>'}}</ul>`;}}
function renderForecastDetail(d){{if(!d||d.error){{$('forecastdetail').innerHTML='';return;}} const f=d.forecast||{{}}, r=d.risk_plan||{{}}, c=d.context||{{}}, res=d.result||{{}}; $('forecastdetail').innerHTML=`<h4>Forecast Detail #${{f.id}}</h4><ul class="list"><li><b>${{f.symbol}}</b> ${{f.interval}} · ${{f.direction}}/${{f.status}} → <span class="${{f.outcome==='pending'?'warn':(res.correct_direction?'good':'bad')}}">${{f.outcome}}</span></li><li>Entry/Stop/Target: <span class="mono">${{fmt(r.entry)}} / ${{fmt(r.stop)}} / ${{fmt(r.target)}}</span> · R/R ${{fmt(r.risk_reward_ratio)}}</li><li>Context: readiness <b>${{c.readiness_score||0}}</b> · council <b>${{c.council_verdict||'n/a'}}</b> · SMC <b>${{c.smc_bias||'n/a'}}</b></li><li>Verified: ${{f.verified_at||'not yet'}}</li></ul><p class="small">${{(d.explanation||[]).join(' ')}}</p>`;}}
