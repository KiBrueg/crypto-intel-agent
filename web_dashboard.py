...
<head>
  ...
  <style>
    :root{{--bg:#fbf7ef;--bg2:#f4ecdf;--paper:#fffdf8;--paper2:#fbf4e8;--ink:#241c16;--text:#332820;--muted:#7c6e60;--faint:#a99a8a;--line:#e7d8c7;--line2:#dcc8b3;--accent:#a85f3d;--accent2:#c9855d;--accent3:#e6b17e;--espresso:#3a2419;--green:#427a55;--red:#b54a3a;--amber:#b7792b;--shadow:rgba(74,45,24,.10);--shadow2:rgba(74,45,24,.16);}}
...
function drawChart(candles, levels){{const c=$('chart'), ctx=c.getContext('2d'), w=c.width,h=c.height; ctx.clearRect(0,0,w,h); const grad=ctx.createLinearGradient(0,0,0,h); grad.addColorStop(0,'#fffdf8'); grad.addColorStop(1,'#f7eddf'); ctx.fillStyle=grad; ctx.fillRect(0,0,w,h); if(!candles||!candles.length)return; const hi=Math.max(...candles.map(x=>x.high)), lo=Math.min(...candles.map(x=>x.low)), span=hi-lo||1, pad=28; const X=i=>pad+i*(w-pad*2)/Math.max(1,candles.length-1), Y=p=>pad+(hi-p)/span*(h-pad*2);
 const all=[...(levels.support_resistance||[]),...Object.values(levels.fibonacci||{{}}),...Object.values(levels.pivots||{{}})]; ctx.font='10px Source Sans 3'; all.forEach((lv,i)=>{{const y=Y(lv); if(y<pad||y>h-pad)return; ctx.strokeStyle=i%2?'#c9855d':'#dcc8b3'; ctx.setLineDash([4,4]); ctx.beginPath(); ctx.moveTo(pad,y); ctx.lineTo(w-pad,y); ctx.stroke(); ctx.setLineDash([]);}});
 candles.forEach((k,i)=>{{const x=X(i), o=Y(k.open), cl=Y(k.close), hh=Y(k.high), ll=Y(k.low), up=k.close>=k.open; ctx.strokeStyle=up?'#427a55':'#b54a3a'; ctx.fillStyle=ctx.strokeStyle; ctx.beginPath(); ctx.moveTo(x,hh); ctx.lineTo(x,ll); ctx.stroke(); ctx.fillRect(x-2,Math.min(o,cl),4,Math.max(1,Math.abs(cl-o)));}});}}
loadSnapshot();
</script></body></html>'''
